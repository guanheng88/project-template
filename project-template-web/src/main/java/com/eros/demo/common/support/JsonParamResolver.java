...
public class JsonParamResolver implements HandlerMethodArgumentResolver {
	 ...
	 * @return
	 */
	private Gson createDefaultObjectGson() {
	...
	private Object parseObject(MethodParameter param, JSONObject json) throws MissingServletRequestParameterException {
		...
		else if (JsonMap.class.isAssignableFrom(param.getParameterType())) {
			// convert json for JsonMap
			jsonValue = "{\"map\":" + jsonValue + "}";
		}

		return gson.fromJson(jsonValue, param.getGenericParameterType());
	}
}
